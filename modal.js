...
const postRegisterModal = document.querySelector(".postRegisterModal");
const postRegisterClose = document.querySelector(".postRegisterClose");
...
postRegisterClose.addEventListener("click", handleClosePostRegisterModal);
btnFermer.addEventListener("click", handleClosePostRegisterModal);
...
function handleClosePostRegisterModal() {
  content.classList.add("hide-modal");
  setTimeout(function () {
    modalbg.style.display = "none";
  }, 222); // timeout for a smoother effect
  postRegisterModal.style.display = "none";
}
...
// Ajoutez une variable pour suivre si le formulaire a été soumis ou non
...
firstNameInput.addEventListener("focus", function () {
  // Réinitialiser l'erreur lorsqu'on se concentre sur le champ
  firstNameError.style.display = "none";
  firstNameInput.classList.remove("field-error");
});

firstNameInput.addEventListener("blur", function () {
  // Valider le champ lorsqu'on le quitte
  if (formSubmitted) {
    validateFirstName();
  }
});
